...
function classify(status,text=''){
 ...
 if(status===400&&/invalid_session|invalid_sections|invalid_image/i.test(value))return'ready';
 if(status===401&&/invalid_api_key/i.test(value))return'ready_key_rejected';
 if((status===401||status===403)&&/permission denied|not authorized|JWT|authorization/i.test(value))return'exists_but_blocked';
 return'error';
}
...
async function probe(name,url,init={}){
 try{
  ...
  return{name,status:response.status,state:classify(response.status,text),detail:text.slice(0,220)};
 }catch(error){return{name,status:0,state:'network_error',detail:String(error?.message||error).slice(0,220)}}
}
...
export default async function handler(req,res){
 ...
 const required=['contact_rpc','schedule_observations_table','schedule_confirm_rpc','schedule_ingest_rpc','schedule_parser_options'];
 const ok=required.every(name=>checks.find(item=>item.name===name)?.state!=='missing'&&checks.find(item=>item.name===name)?.state!=='network_error');
