-- V64.2 hotfix: keep approved derived chunks active while hiding the large parent row.
...
begin
  select * into p from public.uon_ai_knowledge where id = p_parent_id for update;
  if not found or p.source_provider is distinct from 'university_page' or coalesce((p.metadata ->> 'derived_chunk')::boolean, false) then return 0; end if;
  ...
  if v_length <= 1600 then return 0; end if;
  ...
  while v_position <= v_length and v_index < v_total loop
    ...
    if length(v_chunk) >= 80 then
      ...
      insert into public.uon_ai_knowledge(
        ...
        expires_at,content_hash,confidence,verification_status,metadata,last_verified_at,
        updated_at,created_at
      ) values (
        p.title || ' · جزء ' || v_index || '/' || v_total,
        v_chunk,p.category,p.source_url,p.source_title,p.official,v_child_active,
        array(select distinct x from unnest(coalesce(p.tags,'{}'::text[])||array['derived_chunk','v64.2']) as x where x<>''),
        p.source_provider,v_external_id,p.source_type,p.fetched_at,p.source_updated_at,p.expires_at,
        md5(coalesce(p.content_hash,'')||':'||v_index::text||':'||v_chunk),p.confidence,
        p.verification_status,
        coalesce(p.metadata,'{}'::jsonb)||jsonb_build_object(
          'derived_chunk',true,'chunk_version','64.2','parent_id',p.id::text,
          'parent_external_id',p.source_external_id,'chunk_index',v_index,
          'chunk_count',v_total,'chunk_start',v_position,'chunk_chars',length(v_chunk)
        ),
        p.last_verified_at,now(),now()
      )
      ...
      do update set
        ...
        source_url=excluded.source_url,source_title=excluded.source_title,official=excluded.official,
        active=excluded.active,tags=excluded.tags,source_type=excluded.source_type,
        ...
        metadata=excluded.metadata,last_verified_at=excluded.last_verified_at,updated_at=now();
    end if;
  ...
  set active=false,
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('chunked',true,'chunk_version','64.2','chunk_count',v_index),
      updated_at=now()
  where id=p.id;
...
-- Rebuild current children from the hidden parent provenance rows using review state.
do $$
declare r record;
begin
  for r in
    select id from public.uon_ai_knowledge
    where source_provider='university_page'
      and not coalesce((metadata->>'derived_chunk')::boolean,false)
      and coalesce((metadata->>'chunked')::boolean,false)
  loop
