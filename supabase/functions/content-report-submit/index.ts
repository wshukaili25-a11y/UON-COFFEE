...
Deno.serve(async(req:Request)=>{
 ...
 try{
  const body=await req.json().catch(()=>({}));if(body?.dry_run===true)return reply(req,{ok:true,dry_run:true,rate_limit_seconds:15});
  ...
  if(!/^[0-9a-f-]{36}$/i.test(sessionId||''))throw new Error('invalid_session');
  ...
  const{data:slot,error:slotError}=await db.rpc('uon_claim_content_report_slot',{p_session_id:sessionId,p_seconds:15});if(slotError)throw slotError;if(slot!==true)return reply(req,{ok:false,error:'انتظر قليلًا قبل إرسال بلاغ آخر',code:'rate_limited'},429);
