...
function contextualQuestion(body:any,question:string){
  const raw=clean(question,1000).replace(/[؟?!.،,]+$/g,'').trim();
  const q=norm(question).replace(/[؟?!.،,]+$/g,'').trim();
  const history=Array.isArray(body?.history)?body.history:[];
  const previousUsers=[...history].reverse().filter((x:any)=>x?.role==='user'&&clean(x?.content,800)&&norm(x.content)!==norm(question));
  const previous=previousUsers[0];
  ...
  const verification=/(?:تأكد|تاكد|تحقق|شيك|دور|ابحث).*(?:موقع الجامعة|الموقع الرسمي|جامعة نزوى|unizwa|official|website)/i.test(raw)
    || /(?:موقع الجامعة|الموقع الرسمي|unizwa|official|website).*(?:تأكد|تاكد|تحقق|شيك|دور|ابحث)/i.test(raw);

  const shortFollowup=q.length<=100&&/^(?:و|طيب|زين|تمام بس|كم|وين|اين|متى|كيف|وش|ويش|ايش|وعن|وبعدين|بعدها|نفس|هذا|هذي|هاذا|هل|تاكد|تحقق|شيك|دور|ابحث)/i.test(q);

  if(verification)return `${clean(previous.content,600)} — تحقق من موقع جامعة نزوى الرسمي مباشرة`;
...
function casualReply(question:string,language:string){
  ...
  const rows:[RegExp,string,string][]=[
    ...
    [/^(?:(?:هلا|هلا والله|هلا بك|هلو|يا هلا|يا مرحبا|مرحبا|مرحبتين|مرحبا الساع|حياك|حيك|حياك الله|حي الله|الله يحييك)(?:\s+|$))+$/i,'هلا والله 👋 ويش تحتاج؟','Hi 👋 What can I help you with?'],
...
function route(question:string){
  ...
  if(/مدير.*مكتب.*(?:الرئيس|الرئاسة)|مكتب.*(?:الرئيس|الرئاسة).*مدير|director.*(?:chancellor|president).*office/i.test(raw))return 'staff';

  if(/(?:رئيس الجامعة|رئيس جامعه|president of (?:the )?university|university president|chancellor of (?:the )?university)/i.test(raw)
    || /^(?:من|مين|منو|من هو|من هي|who is)\s+(?:رئيس الجامعة|رئيس جامعه|president|chancellor)\b/i.test(raw))return 'search';

  const staffWords=/(?:دكتور|دكتوره|استاذ|استاذه|موظف|عميد|مدير|رئيس|مرشد|doctor|professor|staff|dean|director|advisor)/;
  const staffLookup=(
    /(?:وين|اين|أين|من(?:\s+|$)|مين|منو|من هو|من هي|عطني|اعطني|ابي|ابا|اريد|ابغى|ابغي|دور|ابحث|find|where|who).*(?:دكتور|دكتوره|استاذ|استاذه|موظف|عميد|مدير|رئيس|مرشد|doctor|professor|staff|dean|director|advisor)/.test(q)
  ...
  if(!wantsOfficialVerification(question) && /مبنى|المبنى|قاعه|قاعة|مختبر|خريط|مكان.*(?:جامعه|جامعة|حرم)|موقع.*(?:جامعه|جامعة|حرم)|building|room|lab|campus|campus location/.test(q))return 'campus';

  if(writing||conversational)return 'chat';
  return universitySignal(question)?'search':'chat';
}

function expandStaffQuery(question:string){
  let q=question;
  ...
  const aliases:[RegExp,string][]=[
    ...
    [/رئيس.*قسم|head.*department/i,' Head Department '],
    [/مرشد|advisor/i,' Advisor '],
    [/محاضر|lecturer/i,' Lecturer '],
    [/استاذ|أستاذ|professor/i,' Professor '],
    [/منسق|coordinator/i,' Coordinator '],
    [/اداري|إداري|administrator/i,' Administrator ']
  ];
  for(const [re,extra] of aliases)if(re.test(n)||re.test(question))q+=extra;
  return q;
}
...
async function staffTool(question:string){
  ...
  if(error)throw error;
  const expanded=expandStaffQuery(question);
  const ranked=(data||[]).map((r:any)=>{
    let s=scoreText(expanded,[r.full_name,r.job_title,r.department,r.college].join(' '));
    const target=norm([r.full_name,r.job_title,r.department,r.college].join(' '));
    const q=norm(question);
    if(/مدير.*مكتب.*(?:الرئيس|الرييس)|مكتب.*(?:الرئيس|الرييس)/.test(q) && /director of the chancellor|chancellor.?s office/.test(target))s+=30;
    if(/نائب.*(?:الرئيس|الرييس)/.test(q) && /vice.?chancellor/.test(target))s+=22;
    return {...r,_score:s};
  }).filter((r:any)=>r._score>0)
    .sort((a:any,b:any)=>b._score-a._score||String(a.full_name).localeCompare(String(b.full_name))).slice(0,6);
...
async function generalChat(body:any,question:string,language:string){
  ...
  const liveCue=/حاليا|حالياً|الحين|اليوم|اخر|آخر|احدث|أحدث|الجديد|سعر|اسعار|أسعار|نتيجه|نتيجة|خبر|اخبار|أخبار|latest|current|today|now|price|news|recent|2026/.test(q);
  const factualWho=/^(?:من هو|من هي|من|who is)\s+.{3,}$/i.test(q);
  const useSearch=!likelyWriting&&!likelyStudy&&(liveCue||factualWho);
  const g=await generateWithGemini(system,prompt,.35,1500,useSearch);
  if(!g)return null;
...
function officialUonUrl(value:any){
  try{
    const u=new URL(clean(value,1600));
    return (u.protocol==='https:'||u.protocol==='http:') && (u.hostname==='unizwa.edu.om'||u.hostname.endsWith('.unizwa.edu.om'));
  }catch{return false}
}

async function officialSiteSearch(question:string){
  const started=Date.now();
  try{
    const expanded=expandStaffQuery(question);
    const queryTerms=tokens(expanded).slice(0,10).join(' ') || clean(question,300);
    const url='https://www.unizwa.edu.om/search.php?kw='+encodeURIComponent(queryTerms);
    const r=await fetch(url,{headers:{'user-agent':'UONHub-Agent/1.6 (+https://uonhub.space)'},signal:AbortSignal.timeout(7000),redirect:'follow'});
    if(!r.ok)return {rows:[],trace:tool('uon_official_site_search',started,0,'http_'+r.status)};
    const type=(r.headers.get('content-type')||'').toLowerCase();
    const bytes=new Uint8Array(await r.arrayBuffer());
    const declared=(type.match(/charset=([^;\s]+)/i)?.[1]||'').replace(/["']/g,'');
    let html='';
    try{if(declared)html=new TextDecoder(declared).decode(bytes)}catch{}
    if(!html){
      const utf=new TextDecoder('utf-8').decode(bytes);
      let win=''; try{win=new TextDecoder('windows-1256').decode(bytes)}catch{}
      const arScore=(s:string)=>(s.match(/[\u0600-\u06ff]/g)||[]).length*3-(s.match(/\uFFFD/g)||[]).length*12;
      html=win&&arScore(win)>arScore(utf)?win:utf;
    }
    html=html.slice(0,700000);
    const rows:any[]=[];
    const re=/<a\b[^>]*href\s*=\s*["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;
    let m:any,seen=0;
    while((m=re.exec(html))&&seen<700){
      seen++;
      const rawHref=decodeHtml(m[1]||'');
      if(!rawHref||/^(?:#|javascript:|mailto:|tel:)/i.test(rawHref))continue;
      let href='';
      try{href=new URL(rawHref,'https://www.unizwa.edu.om/').toString()}catch{continue}
      if(!officialUonUrl(href))continue;
      if(/\.(?:css|js|png|jpe?g|gif|svg|ico|woff2?|ttf)(?:\?|$)/i.test(href))continue;
      const title=decodeHtml(String(m[2]||'').replace(/<[^>]+>/g,' '));
      const around=decodeHtml(
        html.slice(Math.max(0,m.index-420),Math.min(html.length,re.lastIndex+900))
          .replace(/<script[\s\S]*?<\/script>/gi,' ')
          .replace(/<style[\s\S]*?<\/style>/gi,' ')
          .replace(/<[^>]+>/g,' ')
      );
      if(!title && !around)continue;
      let score=scoreText(expanded,title+' '+around);
      if(/profile_details\.php/i.test(href) && /دكتور|دكتوره|استاذ|استاذه|موظف|مدير|رئيس|عميد|مرشد|doctor|professor|staff|director|chancellor|dean|advisor/i.test(expanded))score+=8;
      if(/search\.php/i.test(href)||/index\.php\?contentid=1(?:$|&)/i.test(href))score-=8;
      if(score<=0)continue;
      rows.push({type:'بحث موقع الجامعة',title:title||'نتيجة من موقع جامعة نزوى',description:clean(around,850),url:href,official:true,_score:score});
    }
    const unique:any[]=[];
    const seenUrl=new Set<string>();
    for(const row of rows.sort((a:any,b:any)=>b._score-a._score)){
      const key=row.url.replace(/&amp;/g,'&');
      if(seenUrl.has(key))continue;
      seenUrl.add(key); unique.push(row);
      if(unique.length>=8)break;
    }
    return {rows:unique,trace:tool('uon_official_site_search',started,unique.length,unique.length?'ok':'no_match')};
  }catch(e){
    console.error('uon-official-site-search',clean(String(e),500));
    return {rows:[],trace:tool('uon_official_site_search',started,0,'error')};
  }
}

async function fetchOfficialContext(rows:any[]){
  const started=Date.now();
  const urls=[...new Set(rows.map((r:any)=>clean(r.url||r.source_url,1600)).filter(officialUonUrl))].slice(0,3);
  if(!urls.length)return {rows:[],trace:tool('uon_official_web',started,0,'no_urls')};
  const fetched=await Promise.all(urls.map(async url=>{
    try{
      const r=await fetch(url,{headers:{'user-agent':'UONHub-Agent/1.5 (+https://uonhub.space)'},signal:AbortSignal.timeout(6000),redirect:'follow'});
      if(!r.ok)return null;
      const type=(r.headers.get('content-type')||'').toLowerCase();
      ...
      return {type:'صفحة رسمية حية',title,description:body,url,official:true,_live:true};
    }catch{return null}
  }));
  const live=fetched.filter(Boolean);
  return {rows:live,trace:tool('uon_official_web',started,live.length,live.length?'ok':'unavailable')};
}
...
function formatResult(result:any,question:string,language:string){
  ...
  if(result.kind==='staff'){
    if(!rows.length) answer=en?'I could not find a verified match in the current staff directory. Try the full name or department.':'ما حصلت تطابقًا مؤكدًا في دليل الموظفين الحالي. جرّب الاسم الكامل أو اسم القسم.';
      ...
      const r=rows[0];
      const q=norm(question);
      const label=(!en && /مدير.*مكتب.*(?:الرئيس|الرييس)|مكتب.*(?:الرئيس|الرييس)/.test(q))
        ? 'مدير مكتب رئيس الجامعة: '+r.full_name
        : r.full_name;
      const parts=[label,r.job_title,r.department,r.college,r.email?((en?'Email: ':'البريد: ')+r.email):'',r.phone?((en?'Phone: ':'الهاتف: ')+r.phone):'',r.extension?((en?'Ext: ':'التحويلة: ')+r.extension):'',r.office_location?((en?'Office: ':'المكتب: ')+r.office_location):''].filter(Boolean);
...
Deno.serve(async (req:Request)=>{
  ...
  try{
    ...
    if(casual){
      ...
      return reply(req,{answer:casual,links:[],actions:[],request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:'chat',tool_trace:[],grounded:false,confidence:0.99});
    }
    ...
    if(selected==='chat'){
      ...
      if(g?.answer){
        ...
        return reply(req,{...g,request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:'chat',tool_trace:[{name:'general_chat',status:'ok',count:1,ms:0}]});
      }
      const fb=await fallback(req,{...body,question});
      if(fb.ok&&fb.data?.answer)return reply(req,{...fb.data,agent:true,agent_version:'1.8.1',intent:'chat',tool_trace:[fb.trace],fallback:true});
      return reply(req,{answer:language==='en'?'I could not reply just now. Try again in a moment.':'ما قدرت أرد عليك الحين، جرّب مرة ثانية بعد شوي 🙏',links:[],actions:[],agent:true,agent_version:'1.8.1',intent:'chat',tool_trace:[fb.trace],grounded:false,confidence:0.3});
    }
    ...
    if(!hasRows&&!hasContext&&['staff','course','calendar','campus','search'].includes(selected)){
      const site=universitySignal(effectiveQuestion)?await officialSiteSearch(effectiveQuestion):{rows:[],trace:tool('uon_official_site_search',Date.now(),0,'skipped')};
      if(site.rows.length){
        const official=await fetchOfficialContext(site.rows.slice(0,3));
        const synthesis=await groundedUniversityAnswer(question,[...official.rows,...site.rows],language);
        if(synthesis?.answer){
          ...
          return reply(req,{answer:synthesis.answer,links,actions:[],request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:selected,resolved_question:effectiveQuestion!==question?effectiveQuestion:undefined,tool_trace:[result.trace,site.trace,official.trace,{name:'grounded_synthesis',status:'ok',count:official.rows.length+site.rows.length,ms:0}],grounded:true,confidence:.95,sources_count:links.length,used_model:true,ai_provider:'google_gemini',ai_model:synthesis.model,ai_model_version:synthesis.modelVersion});
        }
      }
      ...
      if(web?.answer){
        ...
        return reply(req,{answer:web.answer,links:web.links||[],actions:[],request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:selected,resolved_question:effectiveQuestion!==question?effectiveQuestion:undefined,tool_trace:[result.trace,site.trace,{name:'google_search',status:web.used_search?'ok':'no_results',count:(web.links||[]).length,ms:web.ms||0}],grounded:Boolean(web.used_search),confidence:web.used_search?.93:.68,sources_count:(web.links||[]).length,used_model:true,ai_provider:'google_gemini',ai_model:web.model,ai_model_version:web.modelVersion,web_search_queries:web.search_queries||[]});
      }
      const fb=await fallback(req,{...body,question:effectiveQuestion});
      if(fb.ok&&fb.data?.answer)return reply(req,{...fb.data,agent:true,agent_version:'1.8.1',intent:selected,resolved_question:effectiveQuestion!==question?effectiveQuestion:undefined,tool_trace:[result.trace,site.trace,fb.trace],fallback:true});
    }
    ...
    if(selected==='search'&&hasRows){
      ...
      if(official.rows.length){
        ...
        if(liveSynthesis?.answer && !/ما قدرت|لم أتمكن|غير متوفر|غير متوفرة|could not verify|couldn't verify/i.test(liveSynthesis.answer)){
          ...
          return reply(req,{answer:liveSynthesis.answer,links,actions:[],request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:'search',tool_trace:[result.trace,site.trace,official.trace,{name:'grounded_synthesis',status:'ok',count:official.rows.length+site.rows.length+result.rows.length,ms:0}],grounded:true,confidence:.97,sources_count:links.length,used_model:true,ai_provider:'google_gemini',ai_model:liveSynthesis.model,ai_model_version:liveSynthesis.modelVersion});
        }
      }
      ...
      if(web?.answer){
        ...
        const request_id=await persistDirect(body,question,web.answer).catch(()=>null);
        return reply(req,{answer:web.answer,links,actions:[],request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:'search',tool_trace:[result.trace,site.trace,official.trace,{name:'google_search',status:web.used_search?'ok':'no_results',count:(web.links||[]).length,ms:web.ms||0}],grounded:Boolean(web.used_search||result.rows.length),confidence:web.used_search?.96:.88,sources_count:links.length,used_model:true,ai_provider:'google_gemini',ai_model:web.model,ai_model_version:web.modelVersion,web_search_queries:web.search_queries||[]});
      }
      ...
      if(synthesis?.answer){
        ...
        const request_id=await persistDirect(body,question,synthesis.answer).catch(()=>null);
        return reply(req,{answer:synthesis.answer,links,actions:[],request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:'search',tool_trace:[result.trace,official.trace,{name:'grounded_synthesis',status:'ok',count:result.rows.length,ms:0}],grounded:true,confidence:.9,sources_count:links.length,used_model:true,ai_provider:'google_gemini',ai_model:synthesis.model,ai_model_version:synthesis.modelVersion});
      }
    }

    if(selected==='staff' && hasRows){
      const firstScore=Number(result.rows?.[0]?._score||0);
      const secondScore=Number(result.rows?.[1]?._score||0);
      const ambiguous=result.rows.length>1 && (firstScore<10 || Math.abs(firstScore-secondScore)<=2);
      if(ambiguous && !wantsOfficialVerification(effectiveQuestion)){
        const site=await officialSiteSearch(effectiveQuestion);
        if(site.rows.length){
          const official=await fetchOfficialContext(site.rows.slice(0,3));
          const synthesis=await groundedUniversityAnswer(question,[...site.rows,...official.rows],language);
          if(synthesis?.answer && !/ما قدرت|لم أتمكن|غير واضح|غير متوفر|could not|couldn't/i.test(synthesis.answer)){
            ...
            const request_id=await persistDirect(body,question,synthesis.answer).catch(()=>null);
            return reply(req,{answer:synthesis.answer,links,actions:[],request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:'staff',tool_trace:[result.trace,site.trace,official.trace,{name:'staff_disambiguation',status:'ok',count:site.rows.length,ms:0}],grounded:true,confidence:.96,sources_count:links.length,used_model:true,ai_provider:'google_gemini',ai_model:synthesis.model,ai_model_version:synthesis.modelVersion});
          }
        }
      }
    }

    if(selected==='staff' && hasRows && wantsOfficialVerification(effectiveQuestion)){
      const top=result.rows[0];
      const official=await fetchOfficialContext([top]);
      const officialText=norm((official.rows||[]).map((r:any)=>r.description||'').join(' '));
      const nameTokens=tokens(top.full_name||'').filter((x:string)=>x.length>2);
      const nameHits=nameTokens.filter((x:string)=>officialText.includes(x)).length;
      const emailOk=Boolean(top.email)&&officialText.includes(norm(top.email));
      const extOk=Boolean(top.extension)&&officialText.includes(norm(top.extension));
      const verified=official.rows.length>0 && (emailOk || extOk || nameHits>=Math.min(2,nameTokens.length));

      if(verified){
        const exact=formatResult({...result,rows:[top]},effectiveQuestion,language);
        const prefix=language==='en'
          ? 'Verified directly from the official University of Nizwa staff page ✅\n'
          : 'تأكدت منه مباشرة من صفحة الموظفين الرسمية في موقع جامعة نزوى ✅\n';
        ...
        return reply(req,{answer,links,actions:exact.actions||[],request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:'staff',tool_trace:[result.trace,official.trace,{name:'official_match',status:'ok',count:1,ms:0}],grounded:true,confidence:.995,sources_count:links.length,verified_live:true});
      }
    ...
    if(formatted.answer){
      ...
      return reply(req,{...formatted,request_id:request_id||undefined,agent:true,agent_version:'1.8.1',intent:selected,resolved_question:effectiveQuestion!==question?effectiveQuestion:undefined,tool_trace:[result.trace],grounded:true,confidence:result.rows?.length?0.94:0.72});
    }
    const fb=await fallback(req,body);
    if(fb.ok&&fb.data?.answer)return reply(req,{...fb.data,agent:true,agent_version:'1.8.1',intent:selected,tool_trace:[result.trace,fb.trace],fallback:true});
    return reply(req,{answer:language==='en'?'I could not verify an answer right now. Try a more specific question.':'ما قدرت أتحقق من إجابة دقيقة حاليًا. جرّب سؤال أكثر تحديدًا.',links:[],actions:[],agent:true,agent_version:'1.8.1',intent:selected,tool_trace:[result.trace,fb.trace],grounded:false,confidence:0.35});
  }catch(e){
