...
function validCourseCode(code:string){return /^[A-Z]{2,8}[0-9]{2,4}[A-Z]?$/.test(code);}
...
async function dashboard(){
 ...
 const quarantineIds=new Set(quarantineRows.map((x:any)=>String(x.source_course_id)));
 const cleanCourses=(courses.data||[]).filter((x:any)=>!quarantineIds.has(String(x.id)));
 const linkedCodes=new Set((links.data||[]).map((x:any)=>String(x.course_code||'').toUpperCase()));
 const active=cleanCourses.filter((x:any)=>x.active!==false);
 return {
  courses:cleanCourses,colleges:colleges.data||[],departments:departments.data||[],programs:programs.data||[],links:links.data||[],quarantine:quarantineRows,
  metrics:{total:cleanCourses.length,active:active.length,inactive:cleanCourses.length-active.length,linked:active.filter((x:any)=>linkedCodes.has(String(x.code||'').toUpperCase())).length,unlinked:active.filter((x:any)=>!linkedCodes.has(String(x.code||'').toUpperCase())).length,quarantined:quarantineRows.length}
 };
}
...
async function saveCourse(body:any){
 ...
 if(nameAr.length<2)throw new Error('اسم المقرر بالعربي مطلوب');
 ...
 const {data,error}=await db.rpc('admin_upsert_course_with_programs',{
  p_course:{...course,code,name_ar:nameAr},
  p_program_ids:programIds,
  p_requirement_type:requirement
 });
 if(error)throw error;
 await db.from('admin_audit_log').insert({admin_name:'web-admin',action:'course_upsert_v32',entity:'courses',entity_id:data?.course?.id||null,details:{code,program_ids:programIds,requirement_type:requirement}});
 return data;
}

async function toggleCourse(body:any){
 const id=String(body.id||'');
 if(!id)throw new Error('معرّف المقرر مطلوب');
...
async function deleteCourse(body:any){
 const id=String(body.id||'');
 if(!id)throw new Error('معرّف المقرر مطلوب');
 const {data:course,error:readError}=await db.from('courses').select('id,code,name_ar').eq('id',id).single();
 if(readError)throw readError;
 const {error}=await db.from('courses').delete().eq('id',id);
 if(error)throw error;
 await db.from('admin_audit_log').insert({admin_name:'web-admin',action:'course_delete_v32',entity:'courses',entity_id:id,details:course});
 return {id};
}

async function bulkCourses(body:any){
 const input=Array.isArray(body.rows)?body.rows.slice(0,1000):[];
 if(!input.length)throw new Error('لا توجد مقررات للاستيراد');
 const normalized=input.map((row:any)=>({
  ...
  description:String(row.description||'').trim()||null,active:String(row.active??'true').toLowerCase()!=='false',status:'approved',updated_at:new Date().toISOString()
 })).filter((x:any)=>validCourseCode(x.code)&&x.name_ar.length>=2);
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const client=/required|invalid|مطلوب|غير صحيح|already exists|موجود/i.test(message);
  return reply(req,{ok:false,error:message},client?400:500);
 }
});
