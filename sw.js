const CACHE_NAME='uonhub-pwa-v32-5-3';
const OFFLINE_URL='/offline.html';
const CORE_ASSETS=['/','/index.html','/tools.html','/groups.html','/assistant.html','/offline.html','/manifest.webmanifest','/css/app.css','/css/ui-refresh-v24.css','/css/pwa.css','/js/core.js','/js/v14-ui.js','/js/v20-experience.js','/js/pwa-init.js','/js/assistant.js','/assets/icons/icon-192.png','/assets/icons/icon-512.png'];
...
self.addEventListener('fetch',event=>{
 ...
 if(event.request.mode==='navigate'){
  event.respondWith(fetch(event.request).then(response=>{if(response.ok)caches.open(CACHE_NAME).then(cache=>cache.put(event.request,response.clone()));return response}).catch(async()=>await caches.match(event.request)||await caches.match(OFFLINE_URL)));
  return;
 }
