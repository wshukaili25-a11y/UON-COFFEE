...
export const insert=(t,b,{returning=true}={})=>
 api(t,{
  method:'POST',
  body:b,
...
export async function submitPending(table,body){
 ...
 // whatsapp_groups يستخدم SERIAL / INTEGER
 // لذلك لا نرسل UUID له
 if(!payload.id && table!=='whatsapp_groups'){
...
export function fillCollege(select,{other=false}={}){
 select.innerHTML=
  ...
  (other?'<option value="أخرى">أخرى</option>':'');
}
...
export function adminSession(){
 try{
  return JSON.parse(
   sessionStorage.getItem('uon_admin_session')||'null'
  );
 }catch{
  return null;
 }
...
export function trackClicks(){
 ...
 document.addEventListener('click',event=>{
  ...
  if(link.matches('a[href*="summaries"]')){
   trackEvent('summary_section_open',{
...
export async function loadNotificationCenter(limit=20){
 ...
 try{
  ...
  const rows=await get(
   'site_notifications',
   `select=*&active=eq.true&order=created_at.desc&limit=${limit}`
  );

  target.innerHTML=rows.length
   ?rows.map(x=>`
<a class="notification-item" href="${esc(x.url||'#')}">
...
`).join('')
  ...
  target.innerHTML='<div class="empty">تعذر تحميل الإشعارات</div>';
...
export async function reportBrokenLink({
 ...
 const reason=prompt(
  'ما المشكلة في الرابط؟',
  'الرابط لا يعمل'
 );

 if(reason===null) return false;

 const report=await submitPending(
  'broken_link_reports',
  {
