import {toast,esc,enforceUonMaintenance,watchUonMaintenance} from './core.js?v=62.1.0';
import {STUDY_ACTIVE_KEY,addStudySession,formatStudyDuration,readStudySessions,readStudySettings,saveStudySettings,scheduleCourses,studyStats} from './study-focus-data.js?v=62.1.0';
...
function syncSettings(){settings=saveStudySettings({focusMinutes:Number($('#focusMinutesCustom').value)||25,breakMinutes:Number($('#breakMinutes').value)||5,weeklyGoalMinutes:Number($('#weeklyGoalMinutes').value)||300,autoBreak:$('#autoBreak').checked,lastCourse:course.value})}
...
function renderStats(){
 ...
 $('#focusToday').textContent=formatStudyDuration(stats.todayMinutes);$('#focusWeek').textContent=formatStudyDuration(stats.weekMinutes);$('#focusTodaySessions').textContent=String(stats.todaySessions);$('#focusWeekSessions').textContent=String(stats.weekSessions);
 $('#focusGoalPercent').textContent=`${stats.goalProgress}%`;$('#focusGoalBar').style.setProperty('--goal-progress',`${stats.goalProgress}%`);$('#focusGoalText').textContent=`${formatStudyDuration(stats.weekMinutes)} من ${formatStudyDuration(stats.goalMinutes)}`;$('#focusStreak').textContent=`🔥 ${stats.streakDays} ${stats.streakDays===1?'يوم':'أيام'}`;
 $('#focusCourseStats').innerHTML=stats.topCourses.length?stats.topCourses.map(item=>`<div class="focus-course-stat"><strong>${esc(item.course)}</strong><span>${formatStudyDuration(item.minutes)}</span></div>`).join(''):'<div class="muted">ابدأ أول جلسة وبتظهر إحصائيات المواد هنا.</div>';
 const rows=readStudySessions().slice(0,6);$('#focusHistory').innerHTML=rows.length?rows.map(row=>`<article><div><strong>${esc(row.course||'عام')}</strong><small>${new Date(row.ended_at).toLocaleDateString('ar-OM',{timeZone:'Asia/Muscat',day:'numeric',month:'short'})}</small></div><span>${formatStudyDuration(row.duration_minutes)}</span></article>`).join(''):'<div class="muted">لا توجد جلسات محفوظة بعد.</div>';
}
...
$('#focusMinutesCustom').value=String(settings.focusMinutes||25);$('#breakMinutes').value=String(settings.breakMinutes||5);$('#weeklyGoalMinutes').value=String(settings.weeklyGoalMinutes||300);$('#autoBreak').checked=Boolean(settings.autoBreak);fillCourses();if(state){course.value=state.course||'';kind.value=state.kind||'focus';$('#focusMinutesCustom').value=String(Math.round((state.totalSeconds||1500)/60))}
...
$('#breakMinutes').addEventListener('change',syncSettings);$('#weeklyGoalMinutes').addEventListener('change',()=>{syncSettings();renderStats();toast('تم تحديث هدفك الأسبوعي')});$('#autoBreak').addEventListener('change',syncSettings);course.addEventListener('change',syncSettings);
