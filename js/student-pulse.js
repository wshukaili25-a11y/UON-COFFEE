...
async function newestContent(topic,pref){
 if(topic==='summaries'){const rows=await get('public_summaries',`select=id,title,course_code,college,created_at${collegeFilter(pref.college)}&order=created_at.desc&limit=1`);const row=rows?.[0];return row?{id:String(row.id),created_at:row.created_at,title:'ملخص جديد',body:[row.course_code,row.title].filter(Boolean).join(' • ')||'تمت إضافة مادة جديدة للمكتبة',url:'summaries.html',icon:'📚'}:null}
...
export function runStudentPulse(now=new Date()){
 ...
 if(topics.has('classes')&&klass&&klass.deltaDays===0){const until=klass.startMinutes-clock.minutes,lead=Math.min(180,Math.max(5,Number(pref.class_lead_minutes)||30));if(klass.state==='now')renderBanner({kind:'class',id:`now-${klass.id||klass.course}-${clock.date}`,icon:'🎓',title:`${klass.course} الآن`,body:`${formatClassTime(klass.start)} – ${formatClassTime(klass.end)}${klass.room?` • ${klass.room}`:''}`,url:'schedule.html'});else if(until>=0&&until<=lead){const body=`بعد ${until} دقيقة • ${formatClassTime(klass.start)}${klass.room?` • ${klass.room}`:''}`;renderBanner({kind:'class',id:`soon-${klass.id||klass.course}-${clock.date}`,icon:'⏰',title:`محاضرتك القادمة: ${klass.course}`,body,url:'schedule.html'});notify(`uon-class-${klass.id||klass.course}-${clock.date}`,`محاضرتك القادمة: ${klass.course}`,body,'schedule.html',seen)}}
 const task=nextStudentTask(now);
 if(topics.has('tasks')&&task){const state=taskDueState(task,now),hours=hoursUntilTask(task,now),lead=Math.min(168,Math.max(1,Number(pref.task_lead_hours)||12));const title=task.course?`${task.title} • ${task.course}`:task.title;if(state==='overdue')renderBanner({kind:'task',id:`task-overdue-${task.id}`,icon:'⚠️',title:'مهمة متأخرة',body:`${title} • كان موعدها ${formatTaskDue(task)}`,url:'tasks.html'});else if(hours!==null&&hours>=0&&hours<=lead){const body=hours<1?`باقي أقل من ساعة • ${formatTaskDue(task)}`:hours<2?`باقي حوالي ساعة • ${formatTaskDue(task)}`:`باقي ${Math.ceil(hours)} ساعة • ${formatTaskDue(task)}`;renderBanner({kind:'task',id:`task-soon-${task.id}`,icon:'✅',title:`موعد مهمة قريب: ${title}`,body,url:'tasks.html'});notify(`uon-task-${task.id}-${task.due}`,`موعد مهمة قريب: ${title}`,body,'tasks.html',seen)}}
