import {parsePracticeBackup} from './course-practice-data.js?v=71.0.0';
...
export async function inspectFile(file,{practiceCourse}={}){if(!(file instanceof File))return{safe:false,reason:'not-a-file'};if(file.size<=0||file.size>MAX_FILE_SIZE)return{safe:false,reason:'invalid-size'};if(practiceCourse){if(file.size>400000||!['application/json','text/plain',''].includes(file.type))return{safe:false,reason:'invalid-practice-file'};try{parsePracticeBackup(await file.text(),practiceCourse)}catch{return{safe:false,reason:'invalid-practice-file'}}}else if(!ALLOWED_FILE_TYPES.has(file.type))return{safe:false,reason:'unsupported-type'};const hash=await hashFile(file);return{safe:true,hash,size:file.size,type:file.type,name:file.name.replace(/[<>:"/\\|?*\x00-\x1F]/g,'_').slice(0,120)}}
function watchFileInputs(){document.addEventListener('change',async event=>{const input=event.target;if(!(input instanceof HTMLInputElement)||input.type!=='file'||!input.files?.length)return;for(const file of input.files){const course=new URLSearchParams(location.search).get('code');const practiceCourse=location.pathname.endsWith('/course.html')&&input.name==='practice-backup'&&input.id===`practiceImport-${course}`?course:undefined;const result=await inspectFile(file,{practiceCourse}).catch(()=>({safe:false,reason:'scan-failed'}));if(!result.safe){input.value='';logEvent('blocked-file',{name:file.name.slice(0,120),type:file.type||'unknown',size:file.size,reason:result.reason});input.dispatchEvent(new CustomEvent('uon:file-blocked',{bubbles:true,detail:result}));break}input.dispatchEvent(new CustomEvent('uon:file-scanned',{bubbles:true,detail:result}))}},true)}
function bootPageApiBridges(){if(location.pathname.replace(/\/+$/,'')==='/questions.html')void import('./questions-api-bridge.js?v=67.0.2').catch(error=>console.warn('Questions API bridge skipped',error))}
export function bootSecurityGuard(){bootPageApiBridges();hardenLinks();protectDynamicContent();protectForms();watchFileInputs();window.UONSecurity={inspectUrl,inspectFile,hashFile,getEvents:()=>{try{return JSON.parse(localStorage.getItem(LOG_KEY)||'[]')}catch{return[]}},clearEvents:()=>localStorage.removeItem(LOG_KEY)};logEvent('security-guard-ready',{version:'48.0.0'})}
