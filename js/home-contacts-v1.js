...
function phoneParts(value){
 ...
 if(digits.length===8)return {href:`tel:+968${digits}`,display:`+968 ${digits.slice(0,4)} ${digits.slice(4)}`};
...
function mount(rows){
 ...
 if(!anchor)return;
 const section=document.createElement('section');section.id='uonHomeContacts';section.className='uon-home-contacts';
...
async function load(){
 if(unavailable||loading)return;loading=true;
 try{const rows=await rpc('uon_public_contact_numbers',{});mount(rows)}
 catch(error){
  const message=String(error?.message||error||'');
  if(/schema cache|could not find|uon_public_contact_numbers/i.test(message))unavailable=true;
