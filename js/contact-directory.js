...
async function load(){const roots=[...document.querySelectorAll('[data-contact-directory]')];if(!roots.length)return;try{const rows=await rpc('uon_public_contact_numbers',{});if(!Array.isArray(rows)||!rows.length)throw new Error('empty_contacts');roots.forEach(root=>render(root,rows))}catch(error){console.warn('Contact directory unavailable',error);roots.forEach(root=>{root.innerHTML='';root.hidden=true;root.removeAttribute('aria-busy')})}}
