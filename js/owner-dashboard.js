...
function notify(message,error=false){if(!toast)return;toast.textContent=message;toast.className=`toast show${error?' error':''}`;setTimeout(()=>toast.className='toast',3000)}
...
function render(data,days){
 ...
 const stats=[['الأحداث',period.events||0],['الجلسات',period.sessions||0],['المقررات',totals.courses||0],['الملخصات',totals.summaries||0],['المجموعات',totals.groups||0],['البلاغات المعلقة',totals.pending_reports||0]];
...
async function switchTelegram(action){
 const pass=password();if(!pass)return notify('سجل الدخول أولًا',true);
 const activate=action==='activate';const message=activate?'سيتم تحويل Webhook إلى بوت V42 مع إبقاء الوظائف القديمة كاحتياط. متابعة؟':'سيتم الرجوع فورًا للبوت القديم. متابعة؟';if(!confirm(message))return;
 const button=document.querySelector(activate?'#activateTelegramV42':'#rollbackTelegramV42');button.disabled=true;
 try{const response=await fetch(TELEGRAM_API,{method:'POST',headers:{'Content-Type':'application/json','x-admin-password':pass},body:JSON.stringify({action}),cache:'no-store'});const payload=await response.json().catch(()=>({}));if(!response.ok||!payload.ok)throw new Error(payload.error||'تعذر تحديث Webhook');const status=document.querySelector('#telegramV42Status');status.textContent=activate?'V42 مفعل':'البوت القديم مفعل';notify(activate?'تم تفعيل بوت V42':'تم الرجوع للبوت القديم')}catch(error){notify(error.message,true)}finally{button.disabled=false}
}
...
document.querySelector('#ownerRefresh').addEventListener('click',load);daysSelect.addEventListener('change',load);document.querySelector('#ownerLogout').addEventListener('click',()=>{sessionStorage.removeItem(KEY);location.reload()});document.querySelector('#activateTelegramV42').addEventListener('click',()=>switchTelegram('activate'));document.querySelector('#rollbackTelegramV42').addEventListener('click',()=>switchTelegram('rollback'));
load();
