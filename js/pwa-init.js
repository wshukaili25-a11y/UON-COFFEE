...
function promoteScheduleCard(){
 const primary=document.querySelector('.v18-primary-tools');
 if(!primary)return null;
 document.querySelectorAll('.v18-primary-tools [data-feature="schedule"],.v18-secondary-tools [data-feature="schedule"]').forEach(card=>card.remove());
 const card=document.createElement('a');
 card.href='schedule.html';
 card.dataset.feature='schedule';
 card.dataset.card='schedule';
 card.className='uon-schedule-primary-card';
 card.innerHTML='<span class="v18-tool-icon">📅</span><h3 data-ar="الجدول الدراسي" data-en="Study Schedule">الجدول الدراسي</h3><p data-ar="رتّب محاضراتك واكتشف التعارضات وصدّر جدولك." data-en="Organize classes, detect clashes, and export your schedule.">رتّب محاضراتك واكتشف التعارضات وصدّر جدولك.</p>';
 primary.prepend(card);
 return card;
}
function applyFeatureVisibility(state,root=document){
...
function keepScheduleFirst(state){
 const ensure=()=>{
  const primary=document.querySelector('.v18-primary-tools');
  if(!primary)return;
  let card=primary.querySelector('[data-feature="schedule"]');
  if(!card)card=promoteScheduleCard();
  if(card&&primary.firstElementChild!==card)primary.prepend(card);
  if(card){
   const visible=state?.visibility?.schedule!==false;
   card.hidden=!visible;
...
async function boot(){document.querySelector('#uonPwaUpdate')?.remove();installLegacyDockGuard();promoteScheduleCard();addIndependentNotice();addAiButton();createAdminInstallPanel();await registerServiceWorker();try{const{installLanguageLayer}=await import(`./language-v41.js?v=${APP_VERSION}`);installLanguageLayer()}catch(error){console.warn('Language layer failed',error)}try{const{applyFeatureStates}=await import(`./core.js?v=${APP_VERSION}`);const state=await applyFeatureStates(document);applyFeatureVisibility(state,document);keepScheduleFirst(state)}catch(error){console.warn('Feature state bootstrap skipped',error);keepScheduleFirst(null)}}
window.addEventListener('beforeinstallprompt',event=>{event.preventDefault();deferredInstallPrompt=event;document.querySelector('#uonPwaInstall')?.removeAttribute('disabled')});window.addEventListener('appinstalled',()=>{deferredInstallPrompt=null});window.addEventListener('online',()=>document.body.classList.remove('uon-offline'));window.addEventListener('offline',()=>document.body.classList.add('uon-offline'));if(!navigator.onLine)document.body.classList.add('uon-offline');if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
