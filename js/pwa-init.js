...
async function boot(){
 ...
 try{const{applyFeatureStates}=await import(`./core.js?v=${APP_VERSION}`);const state=await applyFeatureStates(document);applyVisibility(state,document);setTimeout(()=>applyVisibility(state,document),1200)}catch(error){console.warn('Feature state bootstrap skipped',error)}
}
