...
async function loadMarketplacePending(){
 ...
 if(!password){pendingList.innerHTML='<div class="empty">انتهت جلسة الإدارة</div>';return}
 pendingList.innerHTML='<div class="empty">جاري تحميل إعلانات السوق…</div>';
 try{
  const rows=await rpc('uon_admin_pending_marketplace',{p_password:password});
...
document.addEventListener('click',event=>{
 ...
 rpc('uon_admin_moderate',{p_password:password,p_table:'marketplace',p_id:String(id),p_action:action})
