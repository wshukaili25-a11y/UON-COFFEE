...
function renderProvider(){const health=$('#googleProviderHealth'),btn=$('#googleConnectBtn');if(health){health.textContent=googleAuthStatusText(providerStatus);health.dataset.state=providerStatus?.enabled?'ready':providerStatus?.checked?'pending':'unknown'}if(btn&&!currentConnection){btn.disabled=providerStatus?.enabled!==true;btn.textContent=providerStatus?.enabled?'ربط Google':'Google قيد الإعداد';btn.title=googleAuthStatusText(providerStatus)}}
function renderStatus(connection){currentConnection=connection||null;const active=connection?.status==='active';const pill=$('#googleStatusPill'),account=$('#googleAccount'),text=$('#googleStatusText'),connectBtn=$('#googleConnectBtn');if(pill){pill.className=`google-status-pill${active?' active':connection?.status==='error'?' error':''}`;pill.textContent=active?'مربوط':connection?.status==='revoked'?'مفصول':'غير مربوط'}if(account)account.textContent=active?(connection.google_email||'حساب Google مربوط'):'ما فيه حساب Google نشط حاليًا';if(text)text.textContent=active?'الربط خاص بحسابك. تقدر تعرض المواعيد وملفات Drive المسموح بها من الأزرار تحت.':providerStatus?googleAuthStatusText(providerStatus):'الربط اختياري. UON Hub يظل يشتغل طبيعي إذا ما ربطت حسابك.';if(connectBtn){connectBtn.hidden=active;connectBtn.disabled=!active&&providerStatus?.enabled!==true}$('#googleRefreshBtn').hidden=!active;$('#googleDisconnectBtn').hidden=!active;const calendar=active&&connection.calendar_read_enabled===true,drive=active&&connection.drive_file_enabled===true;$('#googleCalendarLoad').disabled=!calendar;$('#googleDriveLoad').disabled=!drive;if(!calendar)empty($('#googleCalendarList'),active?'صلاحية Calendar غير متاحة في هذا الربط.':'اربط Google أولًا لعرض المواعيد.');if(!drive)empty($('#googleDriveList'),active?'صلاحية Drive غير متاحة في هذا الربط.':'اربط Google أولًا لعرض الملفات المتاحة للتطبيق.');renderProvider()}
async function refreshProviderStatus(){providerStatus=await getGoogleAuthStatus({supabaseUrl:SUPABASE_URL,publishableKey:SUPABASE_KEY});renderProvider();if(!currentConnection)renderStatus(null);return providerStatus}
...
async function captureProviderSession(s){if(capturing||!s?.provider_token)return false;capturing=true;try{setBusy(true);const data=await api('connect',{provider_token:s.provider_token,provider_refresh_token:s.provider_refresh_token||''});renderStatus(data.connection);history.replaceState({},document.title,location.pathname);try{await supabase.auth.refreshSession()}catch{}toast('تم ربط Google بشكل آمن');return true}catch(e){toast(String(e?.message||e),true);return false}finally{setBusy(false);capturing=false}}
async function connect(){setBusy(true);try{if(providerStatus?.enabled!==true)await refreshProviderStatus();if(providerStatus?.enabled!==true)throw new Error(googleAuthStatusText(providerStatus));const redirectTo=`${location.origin}/google-connect.html`;const{error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo,scopes:`${CALENDAR_SCOPE} ${DRIVE_SCOPE}`,queryParams:{access_type:'offline',prompt:'consent',include_granted_scopes:'true'}}});if(error)throw error}catch(e){setBusy(false);toast(`تعذر بدء ربط Google: ${e?.message||e}`,true)}}
async function disconnect(){if(!confirm('تفصل Google وتحذف التوكنات المخزنة من UON Hub؟'))return;setBusy(true);try{await api('disconnect');await supabase.auth.signOut({scope:'local'});currentSession=null;renderStatus(null);empty($('#googleCalendarList'),'تم فصل Google.');empty($('#googleDriveList'),'تم فصل Google.');toast('تم فصل Google وحذف التوكنات من Vault')}catch(e){toast(String(e?.message||e),true)}finally{setBusy(false)}}
async function loadCalendar(){setBusy(true);try{const data=await api('calendar-upcoming'),list=$('#googleCalendarList');if(!data.events?.length){empty(list,'ما فيه مواعيد قادمة ضمن النتائج الحالية.');return}list.replaceChildren();for(const event of data.events){const a=document.createElement('a');a.className='google-item';a.href=event.url||'#';a.target=event.url?'_blank':'_self';a.rel='noopener noreferrer';const strong=document.createElement('strong');strong.textContent=event.summary||'موعد';const small=document.createElement('small');small.textContent=[safeDate(event.start),event.location].filter(Boolean).join(' · ');a.append(strong,small);list.appendChild(a)}}catch(e){empty($('#googleCalendarList'),`تعذر تحميل التقويم: ${e?.message||e}`)}finally{setBusy(false)}}
async function loadDrive(){setBusy(true);try{const data=await api('drive-files'),list=$('#googleDriveList');if(!data.files?.length){empty(list,'ما فيه ملفات Drive مصرح بها للتطبيق حاليًا. استخدم/اختر ملفًا مع UON Hub أولًا.');return}list.replaceChildren();for(const file of data.files){const a=document.createElement('a');a.className='google-item';a.href=file.url||'#';a.target=file.url?'_blank':'_self';a.rel='noopener noreferrer';const strong=document.createElement('strong');strong.textContent=file.name||'ملف Google Drive';const small=document.createElement('small');small.textContent=[file.mime_type,safeDate(file.modified_at)].filter(Boolean).join(' · ');a.append(strong,small);list.appendChild(a)}}catch(e){empty($('#googleDriveList'),`تعذر تحميل Drive: ${e?.message||e}`)}finally{setBusy(false)}}
...
async function init(){try{await enforceUonMaintenance()}catch{}try{watchUonMaintenance()}catch{}try{await refreshProviderStatus();const s=await session();if(s?.provider_token)await captureProviderSession(s);await loadStatus()}catch(e){console.warn('google-connect init',e);renderStatus(null)}}
void init();