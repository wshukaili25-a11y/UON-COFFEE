...
async function connect(){setBusy(true);try{const redirectTo=`${location.origin}/google-connect.html`;const{error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo,scopes:`${CALENDAR_SCOPE} ${DRIVE_SCOPE}`,queryParams:{access_type:'offline',prompt:'consent',include_granted_scopes:'true'}}});if(error)throw error}catch(e){setBusy(false);toast(`تعذر بدء ربط Google: ${e?.message||e}`,true)}}
